...
import {z} from 'zod'
import {t as tr} from '../trpc.js'

export const messageRouter = tr.router({
    create: tr.procedure
        .input(
            z.object({
                chatId: z.number(),
    ...
    generate: tr.procedure.query(async function* () {
        const chatHistory = llamaChat.chatWrapper.generateInitialChatHistory()
        chatHistory.push({type: 'user', text: 'Hello!'})
        ...
        // Create a generator for streaming chunks
    ...
    delete: tr.procedure.input(z.number()).mutation(async ({input: messageId}) => {
