...
const sendMessage = async () => {
    ...
    chat.value.messages.push({user: 'user', userType: 'user', text: currentMessage.value, pending: false})
    ...
    chat.value?.messages.push({user: character.value.name, userType: 'assistant', text: '', pending: true}) // TODO add a pending flag
    await db.chats.update(chatId, chat.value)
    ...
    chat.value.messages[chat.value.messages.length - 1].pending = false
    await db.chats.update(chatId, chat.value)
        ...
        <div ref="messagesElement" class="flex-grow overflow-auto px-1 md:px-3">
            <template v-if="chat && chat.messages && chat.messages.length">
                <div
                    ...
                    <button
                        v-if="editModeActive && editModeIndex == index"
                        class="btn btn-square btn-neutral p-1 ml-2"
